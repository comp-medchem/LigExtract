...
for p_i, prot in enumerate(prot_lst):
    ...
    path_hideconda=savepath.split(':')
    path_hideconda=[x for x in path_hideconda if 'anaconda3' not in x]
    path_hideconda=':'.join(path_hideconda)
    rmsd=subprocess.run(f'bash -c "source deactivate; PATH={path_hideconda}; pymol -cq ~/LigExtract/bin/align_pdbs_pockets.py -- {prot_dir}/pdbs_filtered_chains/{prot}"', shell=True, capture_output=True)
    ...
    for c in ligand_centroid_sorted.cluster.unique():
        ...
        subprocess.run(f'bash -c "source deactivate; PATH={path_hideconda}; pymol -cq ~/LigExtract/bin/align_ligs_figures.py -- pocketcluster{c} {prot_dir}/pdbs_filtered_chains/{prot}/aligned_pdbs {pocket_ligs}"', shell=True, capture_output=True)
        os.environ["PATH"]=savepath
    ...
    subprocess.run(f'bash -c "source deactivate; PATH={path_hideconda}; pymol -cq ~/LigExtract/bin/allpockets_figure.py -- {prot_dir}/pdbs_filtered_chains/{prot}/aligned_pdbs {prot}_pockets_hierarch-clusters.txt"', shell=True, capture_output=True)
    os.environ["PATH"]=savepath
