    ...
    if len(notaprotein)==0:
        ...
        for chain in notaprotein:
            ...
            if len(res_atom) > 0: # part/all of the ligand is in ATOM
                res_atom =[f"{a}-{b}-{c}" for a,b,c in res_atom.astype(str)]
                ...
                filtered_lig_het = [lig.df["HETATM"].query(f'residue_name=="{ln[0]}" and chain_id=="{ln[1]}" and residue_number=="{ln[2]}"') for ln in all_linked_atms]
                filtered_lig_atm = [lig.df["ATOM"].query(f'residue_name=="{ln[0]}" and chain_id=="{ln[1]}" and residue_number=="{ln[2]}"') for ln in all_linked_atms]
                
                lig.df['ATOM'] = pd.concat(filtered_lig_atm)
                lig.df['HETATM'] = pd.concat(filtered_lig_het)

                if (len(lig.df["ATOM"])+len(lig.df["HETATM"])) != len(all_linked_atms): sys.exit("all_linked_atoms is not mapping correctly to ATOM/HETATM.")
